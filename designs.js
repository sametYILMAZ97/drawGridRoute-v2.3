...
function invalidColumnInputAlert(){
    if (inputColumns.val() < 1 || inputColumns.val() > 150){
        inputColumns.css('color', 'AliceBlue');
...
function makeGridd(height, width) {
    ...
    function cleaning(){
        var cleanpos = new Set([ ...dirtypos])
        //console.log(stringcleanpos);
        
        let stringCleanPos = Array.from(cleanpos);
        //socket.emit('msg', stringCleanPos);   // ile socket e diziyi gönderdim...
        //let array = [...mySet];
        //console.log(stringCleanPos);
        //console.log(cleanpos);

        /* function sendFunction() {
            socket.emit('msg', stringCleanPos);
        } */
